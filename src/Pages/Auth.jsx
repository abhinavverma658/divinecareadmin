...
function Auth() {
  ...
  const [generateAccessToken, { isLoading: tokenLoading }] =
    useGenerateAccessTokenMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (directToken) {
        ...
        const user = signinData?.admin ||
          ...
          signinData?.data?.admin || {
            name:
              signinData?.data?.user?.firstName &&
              signinData?.data?.user?.lastName
        ...
        try {
          ...
          if (token) {
            ...
            console.log(
              "Token generation failed, using demo login as fallback"
            );
      ...
      if (error?.message?.includes("fetch")) {
        ...
        toast.error(
          "CORS error: Server configuration issue. Please try again."
        );
  ...
  return (
    <section style={{ background: "var(--dark-color)" }}>
      <Container className="p-5 custom-section d-flex justify-content-center align-items-center">
        <ZoomDiv>
          <Card
            className="shadow rounded-4 p-4 text-white"
