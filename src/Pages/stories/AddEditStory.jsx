...
const AddEditStory = () => {
    ...
    try {
      ...
      if (token && token.startsWith("demo-token")) {
        ...
        const mockStoryData = {
          ...
          content:
            "<p>This is a sample story content in demo mode. You can edit this content and see how it works in the story editor.</p>",
          author: "Demo Author",
          date: new Date().toISOString(),
          image:
            "https://via.placeholder.com/600x400/007bff/ffffff?text=Demo+Story+Image",
      ...
      if (storyData && storyData._id) {
        setFormData({
          ...
          date: storyData.date
            ? new Date(storyData.date).toISOString().split("T")[0]
            : "",
  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      formData.append("files", resizedFile); // Use 'files' key for backend
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (token && token.startsWith("demo-token")) {
        ...
        toast.success(
          `Story ${id ? "updated" : "created"} successfully! (Demo Mode)`
        );
        navigate("/dash/stories");
        return;
      }
      ...
      toast.success(
        response.message || `Story ${id ? "updated" : "created"} successfully!`
      );
      navigate("/dash/stories");
    } catch (error) {
  ...
  const isLoading_ =
    isLoading || createLoading || updateLoading || uploadingImage;
