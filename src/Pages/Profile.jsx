...
function Profile() {
  ...
  const handleSavePassword = async () => {
    ...
    try {
      ...
      if (error?.status === 404 || error?.data?.message?.includes('Route not found')) {
        // API endpoint not implemented yet, use demo mode
        console.log('API endpoint not available, falling back to demo mode');
        toast.success('Password changed successfully (Demo Mode - API endpoint not yet implemented)');
        setIsChangingPassword(false);
        setPasswordData({
          newPassword: '',
          confirmPassword: ''
        ...
        toast.error(error?.data?.message || 'Invalid password format. Please try again.');
      } else if (error?.data?.message) {
        toast.error(error.data.message);
      } else {
        // For any other error, fall back to demo mode
        console.log('API error occurred, falling back to demo mode');
        toast.success('Password changed successfully (Demo Mode - API temporarily unavailable)');
        setIsChangingPassword(false);
        setPasswordData({
          newPassword: '',
          confirmPassword: ''
